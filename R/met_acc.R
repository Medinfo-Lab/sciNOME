...
QC_epiData <- function(data,
  ...
  # Clear declaration: Each iteration of the loop is expected to return a numeric(1)
...
Reduce_epiDims <- function(
  ...
  if (any(is.na(meth_mat))) {
    ...
    if (impute_method == "knn") {
      ...
      # [New] Evaluate the performance of K values: calculate RMSE using the masking method and output a message
      # 1. Find complete rows without NAs (select up to 500 rows for quick testing)
      complete_idx <- which(rowSums(is.na(meth_mat)) == 0)

      if (length(complete_idx) > 20) {
        # To improve running speed# To improve running speed, use at most 500 lines of complete data for evaluation
        test_rows <- sample(complete_idx, min(length(complete_idx), 500))
        test_mat <- meth_mat[test_rows, , drop = FALSE]

        # 2. Randomly replace 5% of the data with NA (mask)
        n_elements <- length(test_mat)
        mask_idx <- sample(n_elements, size = max(1, floor(0.05 * n_elements)))
        true_values <- test_mat[mask_idx]

        test_mat_masked <- test_mat
        test_mat_masked[mask_idx] <- NA

        # 3. Perform KNN imputation on the mask matrix (run silently to prevent spam)
        capture.output({
          test_imputed <- impute::impute.knn(test_mat_masked, k = safe_k)$data
        })
        imputed_values <- test_imputed[mask_idx]

        # 4. Calculate the RMSE metric (Root Mean Square Error)
        rmse <- sqrt(mean((true_values - imputed_values)^2, na.rm = TRUE))

        message(sprintf("=> [KNN Evaluation] Masking test for K=%d complete. RMSE = %.5f (Lower is better)", safe_k, rmse))
      } else {
        message("=> [KNN Evaluation] Not enough complete rows to perform RMSE evaluation. Proceeding directly.")
      }
