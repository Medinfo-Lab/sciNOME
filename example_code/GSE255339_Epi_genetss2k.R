...
#epis data----
GSE255339_CpG_meth_level_data <- GSE255339_CpG_genetss2k_methlevel
GSE255339_GpC_meth_level_data <- GSE255339_GpC_genetss2k_methlevel
GSE255339_CpG_meth_data <- GSE255339_CpG_genetss2k_meth
GSE255339_GpC_meth_data <- GSE255339_GpC_genetss2k_meth
...
table(GSE255339_CpG_Epis_sample$group1)
...
E4.5_CpG_genetss2k_DEG_data <- merge(CpG_genetss2k_methlevel_DEG_data,CpG_genetss2k_meth_DEG_data,by.x = "chr",by.y = "chr",all.x = T)
E4.5_GpC_genetss2k_DEG_data <- merge(GpC_genetss2k_methlevel_DEG_data,GpC_genetss2k_meth_DEG_data,by.x = "chr",by.y = "chr",all.x = T)
...
E5.5_CpG_genetss2k_DEG_data <- merge(CpG_genetss2k_methlevel_DEG_data,CpG_genetss2k_meth_DEG_data,by.x = "chr",by.y = "chr",all.x = T)
E5.5_GpC_genetss2k_DEG_data <- merge(GpC_genetss2k_methlevel_DEG_data,GpC_genetss2k_meth_DEG_data,by.x = "chr",by.y = "chr",all.x = T)
...
E6.5_CpG_genetss2k_DEG_data <- merge(CpG_genetss2k_methlevel_DEG_data,CpG_genetss2k_meth_DEG_data,by.x = "chr",by.y = "chr",all.x = T)
E6.5_GpC_genetss2k_DEG_data <- merge(GpC_genetss2k_methlevel_DEG_data,GpC_genetss2k_meth_DEG_data,by.x = "chr",by.y = "chr",all.x = T)

# save.image("GeneTSS2k_Differential_Regional_Data.RData")


#CpG GpC DEG diff methlevel----
#E4.5
E4.5_CpG_DEG_data_choose <- E4.5_CpG_genetss2k_DEG_data %>%
  # mutate(numeric_logFC = readr::parse_number(logFC)) %>%
  filter(!is.na(logFC)) %>%
  filter(p.value < 1e-5)
...
ggplot(data = E4.5_DEG_data_union,mapping = aes(x = CpG.Diff, y = GpC.Diff))+
  ...
  labs(x = "Differential methylation",y = "Differential accessibility",
       title = "AZA vs DAC GeneTSS2k Differential")+
...
ggplot(data = E5.5_DEG_data_union,mapping = aes(x = CpG.Diff, y = GpC.Diff))+
  ...
  geom_hline(yintercept = 0, color = "#FF7F00", linetype = "dashed")+   # 水平参考线（y=50）
  labs(x = "Differential methylation",y = "Differential accessibility",
       title = "AZA vs Unt GeneTSS2k Differential")+
  theme(legend.title=element_blank(),
        # plot.title = element_text(hjust = 0.5,size = 18,face = "bold"),
        plot.title = element_textbox(
          # family = "serif",  # 字体
...
ggplot(data = E6.5_DEG_data_union,mapping = aes(x = CpG.Diff, y = GpC.Diff))+
  ...
  geom_hline(yintercept = 0, color = "#FF7F00", linetype = "dashed")+   # 水平参考线（y=50）
  labs(x = "Differential methylation",y = "Differential accessibility",
       title = "DAC vs Unt GeneTSS2k Differential")+
  theme(legend.title=element_blank(),
        # plot.title = element_text(hjust = 0.5,size = 18,face = "bold"),
        plot.title = element_textbox(
          # family = "serif",  # 字体
