...
# GSE255339
files=($(find RNA_fq -maxdepth 1 -name "*_1.fastq.gz" -printf "%f\n"))
for file in "${files[@]}"
do
	#echo ${file}
	name=${file%_*}
	echo ${name}
	fastp --in1 RNA_fq/${name}_1.fastq.gz --in2 RNA_fq/${name}_2.fastq.gz --detect_adapter_for_pe --cut_right --out1 RNA_trim_fq/${name}_R1.clean.fastq.gz --out2 RNA_trim_fq/${name}_R2.clean.fastq.gz --html qc_reports/${name}_fastp.html --json qc_reports/${name}_fastp.json
done
```
...
# GSE255339
files=($(find RNA_trim_fq -maxdepth 1 -name "*_R1.clean.fastq.gz" -printf "%f\n"))
for file in "${files[@]}"
do
	name=${file%_*}
	echo ${name}
	/work/tools/STAR/bin/Linux_x86_64/STAR --genomeDir ~/genomes/hg19_star --readFilesIn RNA_trim_fq/${name}_R1.clean.fastq.gz RNA_trim_fq/${name}_R2.clean.fastq.gz --runThreadN 5 --outSAMtype BAM SortedByCoordinate --readFilesCommand zcat --outFileNamePrefix bam/${name}_
done
```
...
# GSE255339
/work/tools/featureCounts/subread-2.0.1-Linux-x86_64/bin/featureCounts -T 5 -s 0 -M --fraction -t exon -g gene_id -a /work/genomes/UCSC/hg19/gencode.v41lift37.annotation.gtf -o GSE255339_RNA_counts.txt bam/*_Aligned.sortedByCoord.out.bam
```
...
do
	...
	name=${file%_*}
	echo ${name}
	fastp --in1 NOMe_fq/${name}_1.fastq.gz --in2 NOMe_fq/${name}_2.fastq.gz -y 0 --cut_right --detect_adapter_for_pe --trim_front1 6 --trim_front2 6 --length_required 20 --out1 NOMe_trim_fq/${name}_R1.clean.fastq.gz --out2 NOMe_trim_fq/${name}_R2.clean.fastq.gz --html ${name}_fastp.html --json ${name}_fastp.json
done
```
...
do
	...
	echo ${name}
	mkdir bismark/bam/${name}
	bismark --genome ~/genomes/hg19_bismark --bowtie2 --paralle 4 --non_directional -X 1000 -p 4 --bam -1 NOMe_trim_fq/${name}_R1.clean.fastq.gz -2 NOMe_trim_fq/${name}_R2.clean.fastq.gz --output_dir bismark/bam/${name}
done
...
do
	echo ${file}
	mkdir bismark/deduplicate_bam/${file}
	deduplicate_bismark --bam -p --output_dir bismark/deduplicate_bam/${file} bismark/bam/${file}/${file}_R1.clean_bismark_bt2_pe.bam
done
...
do
	echo ${file}
	mkdir bismark/methylation_extractor/${file}
	bismark_methylation_extractor --gzip --bedGraph --CX --buffer_size 10G --genome_folder ~/genomes/hg19_bismark -o bismark/methylation_extractor/${file} bismark/deduplicate_bam/${file}/${file}_R1.clean_bismark_bt2_pe.deduplicated.bam
	rm bismark/methylation_extractor/${file}/CHG_*
	rm bismark/methylation_extractor/${file}/CHH_*
	rm bismark/methylation_extractor/${file}/CpG_* 
done
...
## 5.separate CpG and GpC coverage files
...
# GSE255339
files=($(find bismark/methylation_extractor -maxdepth 1 -printf "%f\n"))
for file in "${files[@]}"
do
	echo ${file}
	mkdir bismark/coverage2cytosine/${file}
	coverage2cytosine --genome_folder ~/genomes/hg19_bismark --nome-seq --gc --gzip --dir bismark/coverage2cytosine/${file} -o ${file} bismark/methylation_extractor/${file}/${file}_R1.clean_bismark_bt2_pe.deduplicated.bismark.cov.gz
done
